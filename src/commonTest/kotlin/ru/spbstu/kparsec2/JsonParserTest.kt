...
class JsonParserTest {
    ...
    object JsonTokens {
        ...
        val number = NumberGrammar.number.map(JsonParserTest::NumberLiteral)
    ...
    object JsonGrammar {
        ...
        val arr = -syntaxToken('[') + manyExpr.map(JsonParserTest::JsonArray) + -syntaxToken(']')
        val objEntry = zipWith(
            token<Token, StringLiteral>(),
            token<Token>(SyntaxToken.Colon),
            expr
        ) { key, _, value -> key.data to value }
